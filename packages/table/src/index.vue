<!-- 这是一个使用Vue 3和TypeScript编写的组件，用于实现具有编辑功能和分页的表格。 -->
<template>
  <el-table
    :data="tableData"
    v-loading="isLoading"
    ...
    v-bind="$attrs"
  >
    <template v-for="item in tableOption" :key="item.label">
      <el-table-column
        :prop="item.prop"
        :label="item.label"
        :width="item.width"
...
const rowClick = (row: any, column: any) => {
  ...
  if (column.label === actionOption.value!.label) {
    // 在组件外部点击编辑按钮的时候 会设置 editRowIndex为edit
    if (props.isEditRow && props.editRowIndex !== "") {
      ...
      // 重置按钮的标识  update:editRowIndex v-model的简写
